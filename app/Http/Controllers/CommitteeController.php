...
class CommitteeController extends Controller
{
    ...
    public function edit(Committee $committee)
    {
        ...
        return view('committees.edit', compact('committee', 'projects', 'staff'));
    }
    ...
    public function update(Request $request, Committee $committee)
    {
        ...
        // تحديث الأعضاء عبر sync (يحذف القديم ويضيف الجديد)
        $members = collect($data['member_ids'])->mapWithKeys(function ($id) use ($request) {
            return [$id => ['role' => $request->member_roles[$id] ?? 'member']];
        });
        ...
        return redirect()->route('committees.show', $committee)
            ->with('success', $request->getLocale() === 'ar'
                ? 'تم تحديث اللجنة بنجاح'
